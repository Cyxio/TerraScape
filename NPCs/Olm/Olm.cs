...
namespace OldSchoolRuneScape.NPCs.Olm //texture 257x168
{
    ...
    public class Olm : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            npc.lavaImmune = true;
            music = MusicID.LunarBoss; //mod.GetSoundSlot(SoundType.Music, "Sounds/Music/MonkeyBadness");
        ...
        public override void NPCLoot()
        {
            ...
            else
            {
                ...
                if (ch == 0)
                {
                    Item.NewItem(npc.Hitbox, mod.ItemType<Items.Magic.Kodaiinsignia>());
                }
                if (ch == 1)
                {
                    Item.NewItem(npc.Hitbox, mod.ItemType<Items.Dinhbulwark>());
                }
                if (ch == 2)
                {
                    Item.NewItem(npc.Hitbox, mod.ItemType<Items.Twistedbow>());
                }
                if (ch == 3)
                {
                    Item.NewItem(npc.Hitbox, mod.ItemType<Items.Dclaws>());
                }
            ...
            if (state == attk)
            {
                ...
                if (attack == 0) // Basic
                {
                    ...
                    if (timer == 21)
                    {
                        ...
                        if (true)
                        {
                            ...
                            {
                                Projectile.NewProjectile(Mouth, spd * 11, mod.ProjectileType<BasicCrystal>(), 200 / 4, 0f);
                                Projectile.NewProjectile(Mouth, (spd * 11).RotatedBy(MathHelper.ToRadians(-25)), mod.ProjectileType<BasicCrystal>(), 200 / 4, 0f);
                                Projectile.NewProjectile(Mouth, (spd * 11).RotatedBy(MathHelper.ToRadians(25)), mod.ProjectileType<BasicCrystal>(), 200 / 4, 0f);
                            }
                            else
                            {
                                Projectile.NewProjectile(Mouth, spd * 30f, mod.ProjectileType<BasicOrb>(), 300 / 4, 0f, 0, healthMode);
                            }
                        }                     
                    }
                    if (timer > 28)
                ...
                else if (attack == 1) // Spheres
                {
                    if (timer == 21)
                    {
                        ...
                        if (true)
                        {
                            Projectile.NewProjectile(spd + new Vector2(250), Vector2.Zero, mod.ProjectileType<RGBsphere>(), 200 / 4, 0f, 0, 0, 0);
                            Projectile.NewProjectile(spd + new Vector2(250).RotatedBy(MathHelper.ToRadians(120)), Vector2.Zero, mod.ProjectileType<RGBsphere>(), 200 / 4, 0f, 0, 1, 0);
                            Projectile.NewProjectile(spd + new Vector2(250).RotatedBy(MathHelper.ToRadians(240)), Vector2.Zero, mod.ProjectileType<RGBsphere>(), 200 / 4, 0f, 0, 2, 0);
                        }
                    }
                    if (timer > 28)
                    {
                ...
                else if (attack == 2) // Shattering Crystal
                {
                    ...
                    if (timer == 21)
                    {
                        ...
                        {
                            spd.Normalize();
                            Projectile.NewProjectile(Mouth, spd * 30f, mod.ProjectileType<CrystalShatter>(), 300 / 4, 0f);
                        }
                    }
                    if (timer > 28)
                    {
                ...
                else if (attack == 3) // Firebreath
                {
                    ...
                    if (timer == 21)
                    {
                        ...
                        {
                            spd.Normalize();
                            Projectile.NewProjectile(Mouth, spd * 20f, mod.ProjectileType<RGBfire>(), 200 / 4, 0f, 0, Main.rand.Next(3), 0);
                        }
                    }
                    if (timer > 28)
                    {
                ...
                else if (attack == 4) // Lightning
                {
                    ...
                    if (timer == 21)
                    {
                        ...
                        {
                            spd.Normalize();
                            Projectile.NewProjectile(Mouth, (spd * 8f).RotatedBy(Math.PI / 6), mod.ProjectileType<OlmLightning>(), 300 / 4, 0f);
                        }
                    }
                    if (timer > 28)
                    {
                ...
                else if (attack == 5) // Channel
                {
                    ...
                    if (timer % 20 == 0)
                    {
                        if (true)
                        {
                            Projectile.NewProjectile(spd, Vector2.Zero, mod.ProjectileType<OlmChannel>(), 1, 0f, 0, 0, 0);
                        }
                    }
                    if (timer > 60)
                    {
                ...
                else if (attack == 6) // Rocks
                {
                    ...
                    if (timer % 20 == 0)
                    {
                        if (Main.netMode != 1)
                        {
                            Projectile.NewProjectile(spd, new Vector2(0, 700 / 60) * Main.rand.NextFloat(0.9f, 1.5f), mod.ProjectileType<OlmRock>(), 300 / 4, 0f);
                        }
                    }
                    if (timer > 100)
                    {
                ...
                else if (attack == 7) // Firewall
                {
                    ...
                    if (timer == 10)
                    {
                        if (true)
                        {
                            Projectile.NewProjectile(new Vector2(spd.X + 360, spd.Y), Vector2.Zero, mod.ProjectileType<OlmFire>(), 350 / 4, 0f, 0, 77, npc.target);
                            Projectile.NewProjectile(new Vector2(spd.X - 360, spd.Y), Vector2.Zero, mod.ProjectileType<OlmFire>(), 350 / 4, 0f, 0, 77, npc.target);
                        }     
                    }
                    if (timer % 45 == 0 && Main.netMode != 1)
                    {
                        spd = Main.player[npc.target].MountedCenter - Mouth;
                        spd.Normalize();
                        Projectile.NewProjectile(Mouth, spd * 13, mod.ProjectileType<BasicCrystal>(), 200 / 4, 0f);
                    }
                    if (timer > 500)
                    {
                        timer = 0;
            ...
            if (state == stationary)
            {
                ...
                if (attack == 50)
                {
                    ...
                    if (timer % 10 == 0)
                    {
                        ...
                        {
                            spd.Normalize();
                            Projectile.NewProjectile(Mouth, (spd * 12f).RotatedBy(Math.PI / 6).RotateRandom(MathHelper.ToRadians(20)), mod.ProjectileType<OlmLightning>(), 200 / 4, 0f);
                        }
                    }
                    if (timer > 90)
                    {
                    ...
                    if (timer % 75 == 0)
                    {
                        ...
                        {                          
                            if (timer < 100)
                            {
                                ...
                                NPC.NewNPC((int)ps.X, (int)ps.Y, mod.NPCType<Greendragon>());
                                NPC.NewNPC((int)ps.X, (int)ps.Y, mod.NPCType<Reddragon>());
                                NPC.NewNPC((int)ps.X, (int)ps.Y, mod.NPCType<Bluedragon>());
                                NPC.NewNPC((int)ps.X, (int)ps.Y, mod.NPCType<Blackdragon>());
                            }
                        }                     
                    }
                    ...
                    if (timer % 50 == 0)
                    {
                        if (Main.netMode != 1)
                        {
                            ...
                            if (timer < 60)
                            {
                                ...
                                {
                                    Vector2 ps = npc.position + (new Vector2(npc.width, npc.height) * Main.rand.NextFloat(1f));
                                    NPC.NewNPC((int)ps.X, (int)ps.Y, mod.NPCType<Lavadragon>());
                                    NPC.NewNPC((int)ps.X, (int)ps.Y, mod.NPCType<Blackdragon>());
                                }
                            }
                        }
                ...
                if (attack == 53)
                {
                    ...
                    if (timer % 45 == 0 && Main.netMode != 1)
                    {
                        ...
                        spd.Normalize();
                        Projectile.NewProjectile(Mouth, spd * 13, mod.ProjectileType<BasicCrystal>(), 200 / 4, 0f);
                        Projectile.NewProjectile(Mouth, (spd * 13).RotatedBy(MathHelper.ToRadians(-25)), mod.ProjectileType<BasicCrystal>(), 200 / 4, 0f);
                        Projectile.NewProjectile(Mouth, (spd * 13).RotatedBy(MathHelper.ToRadians(25)), mod.ProjectileType<BasicCrystal>(), 200 / 4, 0f);
                    }
                ...
                if (attack == 54)
                {
                    ...
                    {
                        Vector2 ps = npc.position + (new Vector2(npc.width, npc.height) * Main.rand.NextFloat(1f));
                        NPC.NewNPC((int)ps.X, (int)ps.Y, mod.NPCType<Demonicgorilla>());
                    }
