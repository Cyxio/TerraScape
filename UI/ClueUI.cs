...
namespace OldSchoolRuneScape.UI
{
    internal class ClueUI: UIState
    {
        ...
        protected override void DrawSelf(SpriteBatch spriteBatch)
        {
            ...
            if (p.channel)
            {
                if (p.inventory[p.selectedItem].type == ModLoader.GetMod("OldSchoolRuneScape").ItemType<Items.ClueScroll.EasyClue>() ||
                    p.inventory[p.selectedItem].type == ModLoader.GetMod("OldSchoolRuneScape").ItemType<Items.ClueScroll.MediumClue>() ||
                    p.inventory[p.selectedItem].type == ModLoader.GetMod("OldSchoolRuneScape").ItemType<Items.ClueScroll.HardClue>() ||
                    p.inventory[p.selectedItem].type == ModLoader.GetMod("OldSchoolRuneScape").ItemType<Items.ClueScroll.EliteClue>() ||
                    p.inventory[p.selectedItem].type == ModLoader.GetMod("OldSchoolRuneScape").ItemType<Items.ClueScroll.MasterClue>())
                {
