...
{
    public class Bind : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                ...
                if (projectile.Colliding(projectile.Hitbox, target.Hitbox) && target.active && !target.friendly && target.type != NPCID.TargetDummy)
                {
                    ...
                    if (target.boss)
                    {
                        target.AddBuff(mod.BuffType<Buffs.Snared>(), 20);
                    }
                    else
                    {
                        target.AddBuff(mod.BuffType<Buffs.Snared>(), 60);
                    }
                    for (int o = 0; o < 36; o++)
                    {
                        ...
                        Main.dust[dust].velocity = new Vector2(2).RotatedBy(MathHelper.ToRadians(10 * o));
                    }
