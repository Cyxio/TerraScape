...
namespace OldSchoolRuneScape.NPCs
{
    ...
    public class Demonicgorilla : ModNPC
    {
        ...
        public override float SpawnChance(NPCSpawnInfo spawnInfo)
        {
            ...
            {
                return SpawnCondition.HardmodeJungle.Chance * 0.05f;
            }
            return 0;
        }
        public override void NPCLoot()
        {
            ...
            if (ch == 0)
            {
                Item.NewItem(npc.Hitbox, mod.ItemType<Items.Ballistalimbs>());
            }
            if (ch == 1)
            {
                Item.NewItem(npc.Hitbox, mod.ItemType<Items.Ballistaspring>());
            }
            if (ch == 2)
            {
                Item.NewItem(npc.Hitbox, mod.ItemType<Items.Heavyframe>());
            }
            if (ch == 3)
            {
                Item.NewItem(npc.Hitbox, mod.ItemType<Items.Lightframe>());
            }
            if (ch == 4)
            {
                Item.NewItem(npc.Hitbox, mod.ItemType<Items.Monkeytail>());
            }
            if (ch == 5)
            {
                Item.NewItem(npc.Hitbox, mod.ItemType<Items.Zenyteshard>());
            }
        }
        ...
        public override void AI()
        {
            ...
            if (state == Attack)
            {
                ...
                if (timer == 25)
                {
                    ...
                    if (attack < 3)
                    {                      
                        Projectile.NewProjectile(npc.Center, spd, mod.ProjectileType<Gorillarock>(), npc.damage / 4, 0f, 0, 0, 0);
                    }
                    else
                    {
                        Projectile.NewProjectile(npc.Center, new Vector2(10 * npc.direction, -10), mod.ProjectileType<Gorillarock>(), npc.damage / 4, 0f, 0, 1, npc.target);
                        if (attack >= 5)
                        {
                            attack = 0;
                            protection++;
