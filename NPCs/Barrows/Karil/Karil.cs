...
namespace OldSchoolRuneScape.NPCs.Barrows.Karil
{
    public class Karilbag : ModItem
    {
        ...
        public override int BossBagNPC => mod.NPCType("Karil");
        ...
        public override void OpenBossBag(Player player)
        {
            ...
            int helm = ModContent.ItemType<Items.Armor.Karilhelm>();
            int body = ModContent.ItemType<Items.Armor.Karilbody>();
            int legs = ModContent.ItemType<Items.Armor.Karillegs>();
            int wep = ModContent.ItemType<Items.Karilcrossbow>();
            int drop = 0;
            for (int i = 0; i < 200; i++)
            {
                bool allcheck = false;
            ...
            player.QuickSpawnItem(ModContent.ItemType<Items.Ammo.Boltrack>(), Main.rand.Next(250, 750));
        }
    }
    [AutoloadBossHead]
    public class Karil : ModNPC
    {
        ...
        public override void NPCLoot()
        {
            ...
            Item.NewItem(npc.Hitbox, ModContent.ItemType<Karilsummon>());
            if (Main.expertMode)
            {
                npc.DropBossBags();
            }
            else
            {
                ...
                {
                    case 0:
                        drop = ModContent.ItemType<Items.Karilcrossbow>();
                        break;
                    case 1:
                        drop = ModContent.ItemType<Items.Armor.Karilhelm>();
                        break;
                    case 2:
                        drop = ModContent.ItemType<Items.Armor.Karilbody>();
                        break;
                    case 3:
                        drop = ModContent.ItemType<Items.Armor.Karillegs>();
                        break;
                    default:
                        break;
                }
                Item.NewItem(npc.Hitbox, drop);
                Item.NewItem(npc.Hitbox, ModContent.ItemType<Items.Ammo.Boltrack>(), Main.rand.Next(250, 500));
            }
        }
        ...
        public override void AI()
        {
            ...
            if (AI_State == Fly)
            {
                ...
                if (AI_Timer % (75 - 10*healthmod) == 0)
                {
                    ...
                    spd.Normalize();
                    spd *= 25;
                    Projectile.NewProjectile(npc.Center, spd.RotateRandom(MathHelper.ToRadians(10)), ModContent.ProjectileType<Ghostbolt>(), npc.damage / 4, 0f, 0, 0, 0);
                }
            ...
            if (AI_State == Attack)
            {
                ...
                if (attacknum == 1)
                {
                    if (AI_Timer == 3)
                    {
                        ...
                        {
                            Projectile.NewProjectile(npc.Center, spd.RotateRandom(MathHelper.ToRadians(75)), ModContent.ProjectileType<Ghostbolt>(), npc.damage / 4, 0f, 0, 1, 0);
                        }
                    }
                    if (AI_Timer > 5)
                    {
                ...
                if (attacknum == 2)
                {
                    ...
                    {
                        Projectile.NewProjectile(npc.Center, spd.RotateRandom(MathHelper.ToRadians(15)), ModContent.ProjectileType<Ghostbolt>(), npc.damage / 4, 0f, 0, Main.rand.Next(3), 0);
                    }
                    if (AI_Timer > 80)
                    {
                        AI_Timer = 0;
                ...
                if (attacknum == 3)
                {
                    ...
                    if (AI_Timer % 15 == 0)
                    {
                        Projectile.NewProjectile(npc.Center, spd, ModContent.ProjectileType<Ghostbolt>(), npc.damage / 4, 0f, 0, 2, 0);
                    }
                ...
                if (attacknum == 4)
                {
                    ...
                    if (AI_Timer % 3 == 0)
                    {
                        Projectile.NewProjectile(npc.Center, new Vector2(0, 10 * healthmod).RotateRandom(Math.PI * 2), ModContent.ProjectileType<Ghostbolt>(), npc.damage / 4, 0f, 0, 2, 0);
                    }
                    if (AI_Timer > 60 * healthmod)
                    {
                        AI_Timer = 0;
