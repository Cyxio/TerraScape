    ...
    public class WaterwaveP : ModProjectile
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            for (int k = 0; k < projectile.oldPos.Length; k++)
            {
                ...
                spriteBatch.Draw(mod.GetTexture("Projectiles/Wavetrail"), drawPos, new Rectangle(0, 18, 30, 18), color, projectile.rotation, drawOrigin, scale, SpriteEffects.None, 0f);
            }
            return true;
        }
        ...
        public override void AI()
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                ...
                if (projectile.Colliding(projectile.Hitbox, target.Hitbox) && target.active && !target.friendly)
                {
                    ...
                    Projectile.NewProjectile(projectile.Center, new Vector2(0), mod.ProjectileType("WaveExplode"), p.GetWeaponDamage(p.inventory[p.selectedItem]), 0f, projectile.owner, 3);
                    for (int o = 0; o < 18; ++o)
                    {
