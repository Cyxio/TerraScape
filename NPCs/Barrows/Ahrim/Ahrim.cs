...
namespace OldSchoolRuneScape.NPCs.Barrows.Ahrim
{
    public class Ahrimbag : ModItem
    {
        ...
        public override void OpenBossBag(Player player)
        {
            ...
            int helm = mod.ItemType<Items.Armor.Ahrimhelm>();
            int body = mod.ItemType<Items.Armor.Ahrimbody>();
            int legs = mod.ItemType<Items.Armor.Ahrimlegs>();
            int wep = mod.ItemType<Items.Magic.Ahrimstaff>();
            int drop = 0;
            for (int i = 0; i < 200; i++)
            {
                bool allcheck = false;
    ...
    public class Ahrim : ModNPC
    {
        ...
        {
            Item.NewItem(npc.Hitbox, mod.ItemType<Ahrimsummon>());
            if (Main.expertMode)
            {
                npc.DropBossBags();
            }
            else
            {
                ...
                {
                    case 0:
                        drop = mod.ItemType<Items.Magic.Ahrimstaff>();
                        break;
                    case 1:
                        drop = mod.ItemType<Items.Armor.Ahrimhelm>();
                        break;
                    case 2:
                        drop = mod.ItemType<Items.Armor.Ahrimbody>();
                        break;
                    case 3:
                        drop = mod.ItemType<Items.Armor.Ahrimlegs>();
                        break;
                    default:
                        break;
                }
            ...
            if (AI_State == Attack)
            {
                ...
                if (attacknum == 1)
                {
                    if (AI_Timer == 3)
                    {
                        ...
                        spd.Normalize();
                        spd *= 16;
                        Projectile.NewProjectile(npc.Center, spd, mod.ProjectileType<Ghostspell>(), npc.damage / 4, 0f);
                        if (healthmod > 1)
                        {
                            Projectile.NewProjectile(npc.Center, spd.RotatedBy(MathHelper.ToRadians(90)), mod.ProjectileType<Ghostspell>(), npc.damage / 4, 0f);
                            Projectile.NewProjectile(npc.Center, spd.RotatedBy(MathHelper.ToRadians(180)), mod.ProjectileType<Ghostspell>(), npc.damage / 4, 0f);
                            Projectile.NewProjectile(npc.Center, spd.RotatedBy(MathHelper.ToRadians(270)), mod.ProjectileType<Ghostspell>(), npc.damage / 4, 0f);
                        }
                    }
                    if (AI_Timer > 5)
                    {
                ...
                if (attacknum == 2)
                {
                    if (AI_Timer == 3)
                    {
                        ...
                        spd.Normalize();
                        spd *= 12;
                        Projectile.NewProjectile(npc.Center, spd, mod.ProjectileType<Ghostspell>(), npc.damage / 4, 0f, 0, 1, 0);
                    }
                    if (AI_Timer > 5)
                    {
                        AI_Timer = 0;
                ...
                if (attacknum == 3)
                {
                    if (AI_Timer == 3)
                    {
                        Projectile.NewProjectile(npc.Center, new Vector2(0, 5), mod.ProjectileType<Ghostspell>(), npc.damage / 4, 0f, 0, 1, 0);
                        Projectile.NewProjectile(npc.Center, new Vector2(0, -5), mod.ProjectileType<Ghostspell>(), npc.damage / 4, 0f, 0, 1, 0);
                        Projectile.NewProjectile(npc.Center, new Vector2(5, 0), mod.ProjectileType<Ghostspell>(), npc.damage / 4, 0f, 0, 1, 0);
                        Projectile.NewProjectile(npc.Center, new Vector2(-5, 0), mod.ProjectileType<Ghostspell>(), npc.damage / 4, 0f, 0, 1, 0);
                        npc.TargetClosest(true);
                        Player target = Main.player[npc.target];
                        float speedX = target.MountedCenter.X - npc.Center.X;
                        ...
                        spd.Normalize();
                        spd *= 20;
                        Projectile.NewProjectile(npc.Center, spd, mod.ProjectileType<Ghostspell>(), npc.damage / 4, 0f, 0, 66, 0);
                    }
                    if (AI_Timer > 5)
                    {
                        AI_Timer = 0;
                ...
                if (attacknum == 4)
                {
                    ...
                    if (AI_Timer % 2 == 0 && AI_Timer > 40)
                    {
                        ...
                        Projectile.NewProjectile(npc.Center, new Vector2(spd.X / 20, spd.Y / 20).RotatedBy(-45), mod.ProjectileType<Ghostspell>(), 3, 0f, 0, 2, 0);
                        Projectile.NewProjectile(npc.Center, new Vector2(spd.X / 20, spd.Y / 20).RotatedBy(45), mod.ProjectileType<Ghostspell>(), 3, 0f, 0, 3, 0);
                    }
                    if (AI_Timer > 120)
                    {
                        AI_Timer = 0;
                ...
                if (attacknum == 5)
                {
                    ...
                    if (AI_Timer % 8 == 0)
                    {
                        Projectile.NewProjectile(new Vector2(spd.X + Main.rand.Next(1920), spd.Y), new Vector2(0, -17), mod.ProjectileType<Ghostspell>(), npc.damage / 4, 0f, 0, 4, 0);
                    }
                    if (AI_Timer > 120)
                    {
                        AI_Timer = 0;
