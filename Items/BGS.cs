...
namespace OldSchoolRuneScape.Items
{
    public class BGS : ModItem
    {
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            {
                item.noMelee = true;
                Projectile.NewProjectile(player.Center, new Vector2(player.direction, 0), mod.ProjectileType("BGSspec"), (int)(item.damage), item.knockBack, player.whoAmI, 0, 0);
                Main.PlaySound(mod.GetLegacySoundSlot(SoundType.Item, "Sounds/Item/AGSspec"), player.position);
                item.noUseGraphic = true;
                player.AddBuff(mod.BuffType("SpecCD"), 60*14);
            }
