...
namespace OldSchoolRuneScape.Items
{
    public class Lightballista : ModItem
    {
        ...
        public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            if (player.altFunctionUse == 2 && !player.GetModPlayer<OSRSplayer>(mod).SpecCD)
            {
                Projectile.NewProjectile(position + new Vector2(speedX, speedY), new Vector2(speedX, speedY), mod.ProjectileType<Projectiles.Ballistaproj>(), damage, knockBack, player.whoAmI, 1, 0);
