...
namespace OldSchoolRuneScape.NPCs
{
    public class RSGlobalNPC : GlobalNPC
    {
        ...
        public override void NPCLoot(NPC npc)
        {
            if (!Terraria.GameContent.Events.DD2Event.Ongoing)
            {
                if (npc.lifeMax > 5 && !npc.SpawnedFromStatue)
                {
                    ...
                    {
                        int drop = mod.ItemType<Items.Magic.aaRuneessence>();
                        if (Main.hardMode)
                        {
                            drop = mod.ItemType<Items.Magic.aDarkessence>();
                        }
                        if (NPC.downedBoss3)
                        {
                            drop = mod.ItemType<Items.Magic.abPureessence>();
                        }
                        Item.NewItem(npc.Hitbox, drop, Main.rand.Next(5, 11));
                    }
                    if (Main.rand.Next(300) == 0 && Main.player[npc.target].ZoneUnderworldHeight)
                    {
                        Item.NewItem(npc.Hitbox, mod.ItemType<Onyx>());
                    }
                    if (Main.rand.Next(2) == 0)
                    {
                        RuneDrops(npc);
                    }
                    if (Main.rand.Next(200) == 0 || (Main.player[npc.target].GetModPlayer<OSRSplayer>().RingofWealth && Main.rand.Next(100) == 0))
                    {
                        RareDropTable(npc);
                    }
                    if (Main.rand.Next(128) == 0 && OSRSworld.downedOlm)
                    {
                        Item.NewItem(npc.Hitbox, mod.ItemType<Items.ClueScroll.MasterClue>());
                    }
                    else if (Main.rand.Next(128) == 0 && NPC.downedPlantBoss)
                    {
                        Item.NewItem(npc.Hitbox, mod.ItemType<Items.ClueScroll.EliteClue>());
                    }
                    else if (Main.rand.Next(128) == 0 && Main.hardMode)
                    {
                        Item.NewItem(npc.Hitbox, mod.ItemType<Items.ClueScroll.HardClue>());
                    }
                    else if (Main.rand.Next(128) == 0 && NPC.downedBoss3)
                    {
                        Item.NewItem(npc.Hitbox, mod.ItemType<Items.ClueScroll.MediumClue>());
                    }
                    else if (Main.rand.Next(128) == 0)
                    {
                        Item.NewItem(npc.Hitbox, mod.ItemType<Items.ClueScroll.EasyClue>());
                    }
                }
        ...
        private void RuneDrops(NPC npc)
        {
            ...
            {
                int item = mod.ItemType<Items.Magic.Mindrune>();
                if (NPC.downedMechBoss1 && NPC.downedMechBoss2 && NPC.downedMechBoss3)
                {
                    item = mod.ItemType<Items.Magic.Bloodrune>();
                }
                else if (Main.hardMode)
                {
                    item = mod.ItemType<Items.Magic.Deathrune>();
                }
                else if (NPC.downedBoss3)
                {
                    item = mod.ItemType<Items.Magic.Chaosrune>();
                }
            ...
            {
                Item.NewItem(npc.Hitbox, mod.ItemType<Items.Magic.Waterrune>(), Main.rand.Next(1, 6));
            }
            if (Main.player[npc.target].ZoneJungle && NPC.downedBoss3)
            {
                Item.NewItem(npc.Hitbox, mod.ItemType<Items.Magic.Naturerune>(), Main.rand.Next(1, 6));
            }
            if (Main.player[npc.target].ZoneHoly)
            {
                if (NPC.downedMechBoss1 && NPC.downedMechBoss2 && NPC.downedMechBoss3)
                {
                    Item.NewItem(npc.Hitbox, mod.ItemType<Items.Magic.Astralrune>(), Main.rand.Next(1, 6));
                }
                else
                {
                    Item.NewItem(npc.Hitbox, mod.ItemType<Items.Magic.Cosmicrune>(), Main.rand.Next(1, 6));
                }
            }
            if (Main.player[npc.target].ZoneSnow)
            {
                if (NPC.downedMechBoss1 && NPC.downedMechBoss2 && NPC.downedMechBoss3)
                {
                    Item.NewItem(npc.Hitbox, mod.ItemType<Items.Magic.Soulrune>(), Main.rand.Next(1, 6));
                }
                else if (NPC.downedBoss3)
                {
                    Item.NewItem(npc.Hitbox, mod.ItemType<Items.Magic.Lawrune>(), Main.rand.Next(1, 6));
                }
                else
                {
                    Item.NewItem(npc.Hitbox, mod.ItemType<Items.Magic.Bodyrune>(), Main.rand.Next(1, 6));
                }
            }
            if (Main.player[npc.target].ZoneDesert)
            {
                Item.NewItem(npc.Hitbox, mod.ItemType<Items.Magic.Earthrune>(), Main.rand.Next(1, 6));
            }
            if (Main.player[npc.target].ZoneSkyHeight)
            {
                Item.NewItem(npc.Hitbox, mod.ItemType<Items.Magic.Airrune>(), Main.rand.Next(1, 6));
            }
            if (Main.player[npc.target].ZoneUnderworldHeight)
            {
                Item.NewItem(npc.Hitbox, mod.ItemType<Items.Magic.Firerune>(), Main.rand.Next(1, 6));
            }
        }
        private void RareDropTable(NPC npc)
        {
            ...
            if (table == 1) //runes
            {
                ...
                {
                    case 0:
                        drop = mod.ItemType<Items.Magic.Airrune>();
                        break;
                    case 1:
                        drop = mod.ItemType<Items.Magic.Waterrune>();
                        break;
                    case 2:
                        drop = mod.ItemType<Items.Magic.Earthrune>();
                        break;
                    case 3:
                        drop = mod.ItemType<Items.Magic.Firerune>();
                        break;
                    case 4:
                        drop = mod.ItemType<Items.Magic.Bodyrune>();
                        break;
                    case 5:
                        drop = mod.ItemType<Items.Magic.Cosmicrune>();
                        break;
                    default:
                        drop = mod.ItemType<Items.Magic.Mindrune>();
                        break;
                }
            }
            if (table == 2) //ammo
            {
                ...
                switch (drop)
                {
                    ...
                    case 5:
                        ...
                        break;
                    case 6:
                        drop = mod.ItemType<Items.Ammo.Diamondbolt>();
                        break;
                    case 7:
                        drop = mod.ItemType<Items.Ammo.Emeraldbolt>();
                        break;
                    case 8:
                        drop = mod.ItemType<Items.Ammo.Rubybolt>();
                        break;
                    case 9:
                        drop = mod.ItemType<Items.Ammo.Sapphirebolt>();
                        break;
                    default:
                        drop = mod.ItemType<Items.Ammo.Runebolt>();
                        break;
                }
            }
            if (table == 3) //metals
        ...
        public override void OnHitByItem(NPC npc, Player player, Item item, int damage, float knockback, bool crit)
        {
            if (npc.life <= 0 && (player.GetModPlayer<OSRSplayer>().easyClue != 0 || player.GetModPlayer<OSRSplayer>().mediumClue != 0 || player.GetModPlayer<OSRSplayer>().hardClue != 0 || player.GetModPlayer<OSRSplayer>().eliteClue != 0 || player.GetModPlayer<OSRSplayer>().masterClue != 0))
            {
                ...
                if (npc.type == mod.NPCType<Gargoyle>() && item.type == mod.ItemType<Granitemaul>()) { ClueStep(2, 34, player); }
                //copies
                ...
                if (npc.type == mod.NPCType<Goblin>()) { ClueStep(1, 31, player); }
                if (npc.type == mod.NPCType<Greendragon>()) { ClueStep(2, 35, player); }
                if (npc.type == mod.NPCType<Bluedragon>()) { ClueStep(3, 31, player); }
                if (npc.type == mod.NPCType<Reddragon>()) { ClueStep(3, 32, player); }
                if (npc.type == mod.NPCType<Blackdragon>()) { ClueStep(3, 33, player); }
                if (npc.type == mod.NPCType<Abyssaldemon>()) { ClueStep(3, 34, player); }
                if (npc.type == mod.NPCType<Lavadragon>()) { ClueStep(4, 31, player); }
                if (npc.type == mod.NPCType<Smokedevil>()) { ClueStep(4, 32, player); }
                if (npc.type == mod.NPCType<Demonicgorilla>()) { ClueStep(4, 33, player); }
                if (npc.type == mod.NPCType<Olm.Olm>()) { ClueStep(5, 33, player); }
                if (npc.type == mod.NPCType<Elvarg.Elvarg>()) { ClueStep(2, 36, player); }
                if (npc.type == mod.NPCType<Chaoselemental.Chaoselemental>()) { ClueStep(3, 37, player); }
                if (npc.type == mod.NPCType<Barrows.Barrowsspirit>()) { ClueStep(4, 37, player); }
            }
        }
        public override void OnHitByProjectile(NPC npc, Projectile projectile, int damage, float knockback, bool crit)
        {
            ...
            if (npc.life <= 0 && (player.GetModPlayer<OSRSplayer>().easyClue != 0 || player.GetModPlayer<OSRSplayer>().mediumClue != 0 || player.GetModPlayer<OSRSplayer>().hardClue != 0 || player.GetModPlayer<OSRSplayer>().eliteClue != 0 || player.GetModPlayer<OSRSplayer>().masterClue != 0))
            {
                ...
                if (npc.type == mod.NPCType<Saradominwizard>() && projectile.type == mod.ProjectileType<Sarastrike>()) { ClueStep(3, 35, player); }
                if (npc.type == mod.NPCType<Zamorakwizard>() && projectile.type == mod.ProjectileType<Zamorakflame>()) { ClueStep(3, 35, player); }
                if (npc.type == mod.NPCType<Guthixwizard>() && projectile.type == mod.ProjectileType<Guthixclaw>()) { ClueStep(3, 35, player); }
                //copies
                if (npc.type == NPCID.KingSlime) { ClueStep(1, 17, player); }
                if (npc.type == NPCID.EyeofCthulhu) { ClueStep(1, 18, player); }
                if (npc.type == NPCID.GiantWormHead || npc.type == NPCID.GiantWormBody || npc.type == NPCID.GiantWormTail) { ClueStep(1, 10, player); }
