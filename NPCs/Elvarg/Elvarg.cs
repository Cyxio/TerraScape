...
namespace OldSchoolRuneScape.NPCs.Elvarg
{
    ...
    public class ElvargBag : ModItem
    {
        ...
        public override int BossBagNPC => mod.NPCType("Elvarg");
        ...
        {
            Item.NewItem(player.Center, ModContent.ItemType<Tiles.ElvargMusicBoxItem>(), 1, false, 0);
            player.QuickSpawnItem(mod.ItemType("Greendhide"), Main.rand.Next(24, 45));
            player.QuickSpawnItem(ModContent.ItemType<Items.Mysticcomponents>(), Main.rand.Next(6, 14));
            int ch = Main.rand.Next(5);
            if (ch == 0)
            {
                player.QuickSpawnItem(ModContent.ItemType<Items.Accessories.Boltenchant>());
            }
            if (ch == 1)
            {
                player.QuickSpawnItem(ModContent.ItemType<Items.Excalibur>());
            }
        }
    }
    [AutoloadBossHead]
    public class Elvarg : ModNPC
    {
        ...
        public override void NPCLoot()
        {
            ...
            else
            {
                ...
                Item.NewItem(npc.Hitbox, ModContent.ItemType<Items.Mysticcomponents>(), Main.rand.Next(4, 12));
                int ch = Main.rand.Next(8);
                if (ch == 0)
                {
                    Item.NewItem(npc.Hitbox, ModContent.ItemType<Items.Accessories.Boltenchant>());
                }
                if (ch == 1)
                {
                    Item.NewItem(npc.Hitbox, ModContent.ItemType<Items.Excalibur>());
                }
            }
        }
